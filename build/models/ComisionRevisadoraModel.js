...
class ComisionRevisadoraModel {
    async getAll({ limit = undefined, offset = 0, input = "", estado = undefined, posicion = undefined, orden = undefined, orderBy = "a.nombre", // Campo por defecto para ordenar
    ...
    async create({ nombre, apellido, orden, posicion }) {
        const conn = await Database_1.db.getConnection();
        try {
            const result = await conn.query("INSERT INTO comision_revisadora (nombre,apellido,orden,posicion) VALUES (?,?,?,?)", [nombre, apellido, orden, posicion]);
            return result;
        }
        catch (e) {
            console.error(e);
    ...
    async update({ id, nombre, apellido, orden, posicion }) {
        ...
        const query = `UPDATE comision_revisadora SET ${queryFragments.join(", ")} WHERE id = ?`;
        console.log(query);
        queryParams.push(id);
        try {
