...
class TribunalModel {
    ...
    async create({ nombre, apellido, orden, posicion }) {
        const conn = await Database_1.db.getConnection();
        try {
            const result = await conn.query("INSERT INTO tribunal_etica (nombre,apellido,orden,posicion) VALUES (?,?,?,?)", [nombre, apellido, orden, posicion]);
            return result;
        }
        catch (e) {
            console.error(e);
    ...
    async update({ id, nombre, apellido, orden, posicion }) {
        ...
        const query = `UPDATE tribunal_etica SET ${queryFragments.join(", ")} WHERE id = ?`;
        console.log(query);
        queryParams.push(id);
        try {
