...
class PersonalModel {
    async getAll({ limit = undefined, offset = 0, input = "", estado = undefined, area = undefined, orderBy = "id", // Campo por defecto para ordenar
    ...
    async create({ nombre, apellido, telefono, email, cargo, area, }) {
        ...
        const query = `INSERT INTO personal (${fields.join(", ")}) VALUES (${values.join(", ")})`;
        console.log(query);
        console.log(queryParams);
        const conn = await Database_1.db.getConnection();
        try {
