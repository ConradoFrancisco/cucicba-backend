...
export default class AutoridadEticaService {
  private repository: Repository<AutoridadEtica>;

  constructor() {
    const ds: DataSource = getDataSource();
    this.repository = ds.manager.getRepository(AutoridadEtica);
  }

  public async getAll(
    p: ParamsDto
  ): Promise<{ data: EticaDto[]; total: number }> {
    const where: FindManyOptions<AutoridadEtica>["where"] = {
      deletedAt: null,
    };
    if (p.input) {
    ...
    const order: FindManyOptions<AutoridadEtica>["order"] = {};
    if (p.orderBy) {
  ...
  public async createAutoridad(p: EticaDto): Promise<AutoridadEtica | null> {
    const nuevaAutoridad = new AutoridadEtica();
  ...
  public async updateAutoridad(p: EticaDto): Promise<AutoridadEtica | null> {
  ...
  public async setActive(p: ActiveParamsDto): Promise<AutoridadEtica | null> {
    const autoridadExistente = await this.repository.findOne({
      where: { id: p.id },
    });
    if (!autoridadExistente) {
  ...
  public async delete(p: DeleteParamsDto): Promise<AutoridadEtica | null> {
    const autoridadExistente = await this.repository.findOne({
      where: { id: p.id },
    });
    if (!autoridadExistente) {
