...
export class InmobiliariaIlegalService {
  private repository: Repository<Ilegal>;
  ...
  public async getAll(p: ParamsDto) {
    ...
    const order: FindManyOptions<Ilegal>["order"] = {};
    if (p.orderBy) {
    ...
    const infractoresDto: InmobiliariaIlegalDto[] = infractores.map(
      (ilegal) => new InmobiliariaIlegalDto(ilegal)
    );
